...
function resolveMastraBaseUrl() {
  ...
  const namedBaseUrl = cliArguments.find(argument => argument.startsWith('--base-url='))?.split('=', 2)[1];
  const positionalBaseUrl = cliArguments.find(argument => !argument.startsWith('--'));
  const configuredBaseUrl =
    namedBaseUrl || positionalBaseUrl || process.env.MASTRA_PUBLIC_URL?.trim() || DEFAULT_MASTRA_BASE_URL;
...
async function verifySlackBotScopes(botToken) {
  ...
  if (!response.ok || body.ok !== true) {
    throw new Error(`Slack setup check failed: Slack bot authentication returned ${body.error ?? response.status}.`);
  }
  ...
  if (grantedScopes.size === 0) {
    console.warn('WARN Slack bot authenticated, but Slack did not return OAuth scope headers. Check scopes manually.');
    return;
  }
  ...
  console.log(`PASS Slack bot authentication and ${REQUIRED_SLACK_BOT_SCOPES.length} required scopes`);
}
...
async function verifySlackWebhook(baseUrl, signingSecret) {
  ...
  try {
    ...
    throw new Error(`Slack setup check failed: signed webhook returned non-JSON status ${signedResponse.status}.`);
  }

  if (!signedResponse.ok || responseBody.challenge !== challenge) {
    throw new Error(`Slack setup check failed: signed webhook challenge returned status ${signedResponse.status}.`);
  }
